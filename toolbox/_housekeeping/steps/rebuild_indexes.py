...
from core.fs import iter_files, matches_any_glob

STEP_NAME = "Rebuild folder indexes"
...
def run(ctx):
    ...
    for folder in sorted(folders):
        if matches_any_glob(folder, ctx.config.get("protected_globs", [])):
            skipped += 1
            continue
        ...
        block = [start, "", "## Housekeeping Index", ""]
        ...
        if index_path.exists():
            ...
            if start in text and end in text:
                ...
                if new_text != text:
                    updated += 1
                    ctx.info(f"{'Would update' if ctx.dry_run else 'Updating'} managed index block: {index_path}")
                    if ctx.dry_run:
                        ctx.plan_write_text(index_path, new_text, STEP_NAME, description="Update managed folder index block")
                    else:
                ...
                updated += 1
                ctx.info(f"{'Would append' if ctx.dry_run else 'Appending'} managed index block: {index_path}")
                new_text = text.rstrip() + "\n\n" + block_text
                if ctx.dry_run:
                    ctx.plan_write_text(index_path, new_text, STEP_NAME, description="Append managed folder index block")
                else:
                    ctx.backup_file(index_path)
                    index_path.write_text(new_text, encoding="utf-8")
                    ctx.mark_changed(index_path)
            ...
            created += 1
            ctx.info(f"{'Would create' if ctx.dry_run else 'Creating'} index: {index_path}")
            content = f"---\ntitle: \"{folder.name}\"\ntype: index\nstatus: active\ngenerated_by: qilabs-housekeeping\ngenerated_at: \"{ctx.run_id}\"\n---\n\n# {folder.name}\n\n{block_text}"
            if ctx.dry_run:
                ctx.plan_write_text(index_path, content, STEP_NAME, description="Create folder index")
            else:
    ...
    ctx.add_result(STEP_NAME, {
        ...
        "protected_folders_skipped": skipped
    })
