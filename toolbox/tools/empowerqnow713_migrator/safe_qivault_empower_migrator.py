...
V2: skips existing destinations instead of creating duplicate __migrated copies.
...
Run dry-run:
    python .\safe_qivault_empower_migrator.py
V2: skips existing destinations instead of creating duplicate __migrated copies. --root "C:\QiLabs\40_QiVault\30_empowerqnow713" --dry-run

Run safe copy:
    python .\safe_qivault_empower_migrator.py
V2: skips existing destinations instead of creating duplicate __migrated copies. --root "C:\QiLabs\40_QiVault\30_empowerqnow713" --apply --mode copy

Run move only after reviewing copy result:
    python .\safe_qivault_empower_migrator.py
V2: skips existing destinations instead of creating duplicate __migrated copies. --root "C:\QiLabs\40_QiVault\30_empowerqnow713" --apply --mode move
"""
...
def apply_plan(rows: list[PlanRow], root: Path, mode: str, apply: bool) -> list[PlanRow]:
    """
    V2 behavior: do NOT create __migrated duplicates by default.

    If the planned destination already exists, skip it and log it.
    This is cleaner for repeat runs and partial migrations.
    """
    for row in rows:
        ...
        # Key change: skip existing destination instead of making duplicate renamed copies.
        if row.destination.exists():
            row.status = "would_skip_existing" if not apply else "skipped_existing"
            continue
        ...
        if mode == "copy":
            ...
            shutil.move(str(row.source), str(row.destination))
            row.status = "moved"
        else:
