...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("QiVault EmpowerQNow713 Interactive Migrator V2 - Skip Existing")
        ...
        self.status_var = tk.StringVar(value="Ready. V2 skips existing files instead of creating duplicate __migrated copies.")
        self._build()
        ...
        if not apply:
            ...
            for row in rows:
                if row.action in {"move", "review"}:
                    ...
                    label = "WOULD SKIP EXISTING" if row.status == "would_skip_existing" else f"WOULD {mode.upper()}"
                    self.log(f"{label}: {src} -> {dst} | {row.reason}")
                    shown += 1
